...
public class Factibilizador 
{
	...
	public int srcCost = 0;

	static ArrayList<Double> crosstime = new ArrayList<>();
	static ArrayList<Double> shifttime = new ArrayList<>();
	static ArrayList<Double> swaptime = new ArrayList<>();
	double ant_plan = 0;
	boolean choose = true;

	public Factibilizador(Instancia instancia,Config config, BuscaLocalIntra buscaLocalIntra, int randNum)
	{
	...
	private void executa() 
	{
		...
		while(topMelhores>0)
		{
			...
			Arrays.sort(melhoras,0,topMelhores);  //java中不sort的话，数组中会存在空值
		...
		do
		{
			for (int i = 0; i < NumRotas; i++) 
			{
				if(!auxSai.rota.isFactivel()&&rotas[i].isFactivel())
				{
					...
					ganho=avaliadorFac.ganhoSHIFT(auxSai, auxEntra);  //前者表示转移的点，后者表示被转移的路径
					if(ganho>0)
					{
						srcCost += 1;
						...
						if(custoAvaliacao<noMelhora.custoAvaliacao)  //只选择两条路径之间最优的改进
						{
							if(!noMelhora.ativo)
								melhoras[topMelhores++]=noMelhora;  // 可行调整都加入到melhoras中
	...
	private void BuscaLocalIntra(Rota rota)
	{
		...
		srcCost += buscaLocalIntra.srcCost;
	}
