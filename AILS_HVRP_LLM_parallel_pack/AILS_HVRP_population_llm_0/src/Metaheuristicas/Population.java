...
//        timeLimit = 0.5;
    ...
    public static void main_rush(String[] args) throws ExecutionException, InterruptedException, IOException {

        String[] problems = new String[]{"HVRPFD", "HVRPD", "FSMF", "FSMD", "FSMFD"};
        String index = "0";
        for (int i = 0; i < args.length-1; i+=2)
        {
            if ("-rand".equals(args[i])) {
                randNum = Integer.parseInt(args[i + 1]);
                break;
            }
        ...
        for (int x = 0; x < 5; x++){
            ...
            String methodType = "LLM-AILS";
            String problemType = "Classic";  // Classic, bigTSP, largeIns
            String resultPath = "F:/XZL/AILS_HVRP_LLM_rush_leadboard_park/AILS_HVRP_population_llm_" + index + "/src/";
            String resultFolder = "resultLLM_v0";
            String convergeFolder = "convergeDataRecord_rush_to_best";
//          String resultFileName = timeLimit + "s_" + methodType + "_" + problemType;
//			String resultFileName = timeLimit + "its_" + methodType + "_" + problemType;
            String resultFileName = "scale_" + methodType + "_" + problemType;
            String resultFilePath = resultPath + resultFolder + "/" + problem + "/" + problem + "_" + resultFileName + "_LLM_rush_leaderboard.xlsx";
            ...
            for (int ii = 0; ii < size; ii++) {
                ...
                String insName = new File(leitor.getFile()).getName();
                Instancia instancia = new Instancia(leitor.getFile(), leitor.getConfig(), leitor.isRounded(), leitor.getVariant());
                timeLimit = instancia.getSize() * 10;
                Solucao b = new Solucao(instancia, leitor.getConfig(), randNum);

                // run instances repeatedly
                for (int jj = 0; jj < repeat; jj++) {
                    System.out.println("Run No." + ii + " " + insName + " instance, " + "On repeat No." + jj + " time：");
                    Population igas = new Population(instancia, leitor.getConfig(), leitor.getBest(), leitor.getTimeLimit(), randNum);
...
//					igas.plot.plot2DPopulation(resultPath + resultFolder + "/" + problem + "/" + problemType + "/plotDataRecord/", "Population Distribution", "file " + ii, jj);
                    igas.storeValue(ii, jj, resultPath + resultFolder + "/" + problem + "/" + problemType + "/" + convergeFolder + "/");
