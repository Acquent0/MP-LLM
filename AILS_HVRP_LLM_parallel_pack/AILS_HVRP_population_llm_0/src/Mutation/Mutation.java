...
public class Mutation{
    ...
    public Solucao muSolucao, son;
    protected Rota rotas[];
    protected int NumRotas;
    protected No solucao[];  //表示解中的点的信息， 其中Ponto是点的属性信息，而No表示的是点相关的其他信息，比如点的前驱和后继
    ...
    public Mutation(Instancia instancia, Config config, int randNum){
    ...
    public void setMutation(Solucao s){
        son.clone(s);
    }

    protected void setSolucao()
    {
        this.NumRotas=son.getNumRotas();
        this.rotas=son.rotas;
        this.solucao=son.getSolucao();
        this.f=son.f;
        if (son.factivel) {
    ...
    public Solucao changeVehicles(){
        setSolucao();

        do
        {
            //perturbacaoEscolhida=perturbadores[rand.nextInt(perturbadores.length)];  //randomly select a perturbation
            son.perturbacaoEscolhida=son.perturbadores[rand.nextInt(3)];
            son.perturbacaoEscolhida.perturbar(son);
            factibilizou=factibilizador.factibilizar(son);
        }
        while (!factibilizou);
    ...
    protected void VariandoNumRotas(Solucao s)
    {
        ...
        if(rand.nextDouble()<alfa)
        {
            ...
            {
                boolean aumenta=rand.nextBoolean();
                if(aumenta)
                    this.NumRotas-=passo;
                else
                    this.NumRotas+=passo;
        ...
        if(rand.nextDouble()<alfa)
        {
            ...
            for (int i = 0; i < varicaoQtnRotas; i++)
            {
                ...
                else
                {
                    ...
                    auxLabel = rotas[indexA].label;

                    veiculoAuxVeiculo.setType(veiculoA.getType());
                    ...
                    if(indexA<NumRotas){
                        ...
                        rotas[indexA].label = rotas[indexB].label;
