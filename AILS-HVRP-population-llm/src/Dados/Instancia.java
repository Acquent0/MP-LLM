		...
		try
		{
			...
			while(!str[0].trim().equals("EOF"))
			{
				switch(str[0].trim())
				{
					...
					case "CAPACITY": setCVRP(str[1].trim());break;
				}
				str=in.readLine().split(":");
			}
			...
			if(variante==Variante.FSMD||variante==Variante.FSMF||variante==Variante.FSMFD)
			{
				...
				numRotasMax=size;
				numRotasMax=0;
				numRotasMax = numRotasMin * 2;  // not set number of vehicle
				for (int i = 0; i < tiposVeiculos.length; i++)  // else
...
//	}
	...
	public void setCVRP(String in){
		numTiposVeiculos = 1;
		tiposVeiculos=new TipoVeiculo[numTiposVeiculos];
		for (int i = 0; i < tiposVeiculos.length; i++) {
			tiposVeiculos[i] = new TipoVeiculo(i);
			tiposVeiculos[i].setQ(Integer.valueOf(in));
		}
	}

	public void setTV(){
		for (int i = 0; i < tiposVeiculos.length; i++) {
			tiposVeiculos[i].setR(1);
			tiposVeiculos[i].setF(0);
			tiposVeiculos[i].setM(9999);
		}
	}
